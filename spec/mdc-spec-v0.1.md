...
- **L2 — Mutate.** Additionally pass every `fmt/` case under C-4, every `mutate/` case under C-5, and every `cut/` and `add/` case (CLI-driven: `op.json`/`args.json` argv against `input.mdc.md`, asserting `expected.mdc.md` byte-for-byte or the stated exit code), with the exit-code law: `0` success, `1` usage/IO/parse/not-MDC, `2` domain refusal. `[fmt/already-canonical]` `[mutate/check]` `[mutate/claim-conflict]` `[cut/from-canonical-template]` `[add/append-basic]`
