...
- **ITEM-4.** Nested task items are children of the nearest enclosing task item, per CommonMark list nesting; the model holds them in `children` in document order. A task nested under a *non-task* (prose) list item has no task parent — it is a top-level model item — but its canonical indentation follows its true source list depth, so `fmt` and mutations never dedent or re-parent it. `[parse/nested-children]` `[parse/deep-nesting]` `[parse/prose-nested-task]` `[fmt/child-indent]` `[fmt/prose-nested-indent]`
...
- **L2 — Mutate.** Additionally pass every `fmt/` case under C-4, every `mutate/` case under C-5, and every `cut/`, `add/`, and `note/` case (CLI-driven: `op.json`/`args.json` argv against `input.mdc.md`, asserting `expected.mdc.md` byte-for-byte or the stated exit code), with the exit-code law: `0` success, `1` usage/IO/parse/not-MDC, `2` domain refusal. `[fmt/already-canonical]` `[mutate/check]` `[mutate/claim-conflict]` `[cut/from-canonical-template]` `[add/append-basic]` `[note/basic]`
