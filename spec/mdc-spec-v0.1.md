...
- **ITEM-3.** Only a **trailing** `{…}` block, preceded by at least one space, is metadata. Braces anywhere else in the line are ordinary text. A line whose entire text would be the block has no attribute block — an item must have text — so the braces remain text. Extraction MUST be quote-aware: the block opens at the last **top-level unquoted** `{`, so a brace inside a quoted value (`{x-path="a{b}c" #b}`) does not open a block and a naive "last `{`" scan is wrong (it would mis-split such a line). `[parse/braces-mid-line]` `[fmt/quote-brace-value]`
