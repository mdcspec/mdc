...
13. **Write a second-language conformance runner** (Python — lowest friction, matching CommonMark's `spec_tests.py`) that consumes *only* the manifest + fixtures + CLI contract, no JS. If it can drive the existing JS CLI *and* be re-pointed at another binary by changing one command path (`toml-test`'s `-decoder=<cmd>` model), the contract is proven portable.
14. **Land one genuinely independent implementation** in a different language (Rust/Go/Python) that passes the corpus. This is the highest-value single action: it proves the spec is implementable from prose + corpus alone, surfaces ambiguities the reference silently resolved, and is the exact line Markdoc never crossed and CommonMark/TOML/MDX did. **This triggers the governance graduation** (Phase 0 #3).
15. **Pin the CommonMark surface** (gap G7): declare the normative CommonMark version + GFM extensions MDC assumes, and add adversarial fixtures for the corners MDC actually depends on (tab-indented children, lazy continuation, `~~` tokenization boundaries, inline-markdown stripping in slug generation) — where two CommonMark libraries diverge.
