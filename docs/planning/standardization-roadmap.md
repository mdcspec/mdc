...
14. **A second implementation exists and passes CONF-L1** — `impls/mdc-py/` is a from-scratch, stdlib-only, line-oriented Python parser (no CommonMark library) that passes all 44 L1 cases via `run.py`, **written without consulting the JS reference** (spec + contract + corpus were sufficient — a strong signal on the G7 concern). This proves the spec is implementable in another language and validates the conformance infrastructure end-to-end. *Caveat on the governance trigger:* it is a **second-language reference implementation authored within the project**, not a genuinely third-party one — so it clears the risk register's technical "implementable twice" gate, but the Phase-0 governance graduation still awaits an *external* implementer (or 3+ orgs). Remaining to fully close #14: extend `impls/mdc-py` to **CONF-L2** (fmt/mutate/add/note/edit/cut), and court a third-party implementation.
