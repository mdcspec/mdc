...
When you discover work that is not yet on the list, `add` it rather than hand-editing the file — that keeps the line canonical. Like every mutation, `add` echoes the resulting line (e.g. `- [ ] Fix flaky test {#fix-flaky-test}`); the new id is the `#…` token, ready to `claim`/`check`. Pass `--id <slug>` to choose the id yourself (recommended for any title with punctuation, since generated ids strip it), or let it generate one from the text. By default `add` appends at the end of the file; because a later `.gate` blocks everything after it in document order, use `--section <heading>` or `--after <id>` to place a discovered task where it belongs instead of trapping it behind a gate.
